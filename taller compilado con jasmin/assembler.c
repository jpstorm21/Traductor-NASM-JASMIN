...
void genPredicate(struct nodeType *node, char *nlabel, char *fname) {
  ...
  case OP_GE:
  case OP_LE: {
    struct nodeType *Lexp = nthChild(1, node); struct nodeType *Rexp = nthChild(2, node); assembler(Lexp, fname); assembler(Rexp, fname);
...
void assembler(struct nodeType *node, char *fname) {
  switch(node->nodeType) {
  case NODO_PROGRAMA: { depth = label_num = 0; 
      genHeader(fname); int varnum = 10; genGlobalVar(); genMethodDeclar(); assembler(nthChild(5, nthChild(3, node)), fname);
      genBodyHead(fname, varnum); genAllocArray(0, fname); assembler(nthChild(4, node), fname); genBodyEnd();
      return; 
  }
  case NODO_SUBPROGRAMA: {
    ...
    genAllocArray(depth, fname); assembler(stmt, fname); genPostLude(nameNode->string); depth = 0;
  ...
  case NODO_OPERACION: { 
    ...
    case OPERACION_DIVISION: {
      struct nodeType *Lexp = nthChild(1, node); struct nodeType *Rexp = nthChild(2, node); assembler(Lexp, fname); assembler(Rexp, fname);
      switch(Lexp->valueType) {
      case TypeInt:{ switch(node->op) { case OPERACION_SUMA: print("iadd"); break; case OPERACION_RESTA: print("isub"); break; case OPERACION_MULTIPLICACION: print("imul"); break; case OPERACION_DIVISION: print("idiv"); break; } break;}
      case TypeReal:{ switch(node->op) { case OPERACION_SUMA: print("dadd"); break; case OPERACION_RESTA: print("dsub"); break; case OPERACION_MULTIPLICACION: print("dmul"); break; case OPERACION_DIVISION: print("ddiv"); break; } break;}
  ...
  case NODE_ASSIGN_STMT: {
    ...
    case TypeReal: {
      ...
      case 1: {
	genArrayLHS(LHS, fname); assembler(RHS, fname); fprintf(stdout, "\tdastore\n"); return; }
      }}
    case TypeChar: {
      int check = checkReference(LHS);
      switch(check) {
      case 0: { int index = genSimpleLHS(LHS); assembler(RHS, fname);
      if(index == -1) fprintf(stdout, "\tputstatic %s.%s\n", fname, genField(LHS->string)); else fprintf(stdout, "\tastore %d\n", index); return; }
      case 1: { genArrayLHS(LHS, fname); assembler(RHS, fname); fprintf(stdout, "\taastore\n"); return; }
      }}
  ...
  case NODE_IF: {
    ...
    assembler(ifstmt, fname); fprintf(stdout, "\tgoto %s\n", endlabel);
    fprintf(stdout, "%s:\n", nlabel);
    assembler(elsestmt, fname);
    fprintf(stdout, "%s:\n", endlabel);    
    return; 
  }
  case NODE_WHILE: {
  ...
  struct nodeType *child = node->child; if(child != 0) { do { assembler(child, fname); child = child->rsibling; } while(child != node->child); }
}
